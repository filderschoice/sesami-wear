...
object HelpContent {
    ...
    /**
     * モバイル回線のときだけ失敗する場合の対処（BL-192）。
     *
     * 2026-09-21の実機検証で、端末の「バックグラウンドデータの制限」が本アプリに掛かっていると、
     * モバイル回線が既定の経路のときにウィジェット・タイルからの操作が名前解決の失敗になることを
     * 確認した。アプリを前面にしている間とWi-Fiでは成功するため、利用者からは
     * 「モバイル回線のときだけ、たまに失敗する」としか見えず、電波を疑っても解決しない。
     * ウィジェットの文言も、この条件に当てはまるときだけ「端末の設定を確認」へ変わる
     * （[com.sesamiwear.core.SesameStatusFailure.BACKGROUND_RESTRICTED]）。
     */
    val mobileData =
        HelpTopic(
            id = "mobile-data",
            title = "モバイル回線のときだけ失敗する",
            paragraphs =
                listOf(
                    "ウィジェットやタイルからの操作は、アプリを開いていない状態（バックグラウンド）で" +
                        "通信します。端末の設定でこのアプリのバックグラウンドデータが制限されていると、" +
                        "モバイル回線のときだけ操作がすぐに失敗します。" +
                        "Wi-Fiに繋がっている間とアプリを開いている間は成功するため、" +
                        "「モバイル回線のときだけ、たまに通信エラーになる」ように見えます。",
                    "このときウィジェットには「通信エラー（端末の設定を確認）」と表示されます。" +
     ...
     * 通信の困りごと（Bluetooth・モバイル回線）は末尾へまとめる（BL-192）。
     */
    val topics: List<HelpTopic> =
        listOf(credentials, apiLimit, demo, afterRegistration, widget, bluetooth, mobileData)
}
