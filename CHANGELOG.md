...
- **BL-198（完了）**: Google Play掲載用のスクリーンショットを、デバッグ版の撮影モード（BL-212〜BL-214）を
  使って0.14.0の表示へ撮り直しました。実物のセサミは操作しておらず、実デバイス名も写っていません。
  - Wear OS用: `wear_2`〜`wear_4`（タイルの各状態）と `wear_6`（デバイス選択）をPixel Watch 2実機で
    撮り直しました（2026-09-25）。`wear_5`（解錠確認）は表示が変わっていないため据え置き、
    `wear_1`（コンプリケーション）は撮影時の文字盤が手元に無いため2026-09-06撮影のままとしました
    （ユーザー判断）。
  - スマートフォン用: 0.14.0で表示が変わったホーム画面ウィジェットの `phone_1`〜`phone_3` をPixel 8 Pro実機で
    撮り直しました（2026-09-26）。実機の状態が写るステータスバーとドックはモザイク、壁紙はウィジェットと
    ダイアログ以外をぼかしています。アプリ内の画面の `phone_4`〜`phone_7` は表示が変わっておらず、
    デバッグ版では撮影モードの表示が写るため据え置きました。
  - `docs/store/README.md` の撮影方法・撮影日と、`docs/store/STORE_LISTING.md`「スクリーンショット」を
    更新しました。Play Consoleの画像の差し替えは0.14.0の配信時にユーザーが行います。
- Skill `store-screenshot-masking` を新設しました。実機で撮った掲載用スクリーンショットへの
  モザイク・ぼかしと9:16化の手順と、加工スクリプト `mask_screenshot.py`（隠す範囲の境界を色の連続区間で
  求める `regions` と、加工する `mask`）を持ちます。Skill `store-screenshot-capture` から参照しています。

## 2026-09-23（ガードレール分割のレビュー完了、BL-203）

- **BL-203（完了・クローズ）**: ガードレール配布元 v0.26.0 の追従（PR #59）で行った `rules/` 配下の変更
  （`guardrails-unified.v1.md` の圧縮と `guardrails-app.v1.md` の新設）について、`CONTRIBUTING.md`
  「レビュー要件」に従うセキュリティ観点・プライバシー観点の確認をメンテナーが完了しました。
  PR #59 は規定ブランチへマージ済みのため、BACKLOG.mdから削除しています。

## 2026-09-23（人手検証の分担とリリース単位の検証手順書）

コンテキストの消費を抑えるため、人手検証の分担を「実機へのインストールと手順書の作成までを Claude Code、
検証の操作をユーザー」へ改めました（ユーザー判断）。

- Skill `release-verification-prep` を新設しました。adb接続、デバッグ版のインストールと結果の確認、
  前提のテキストでの確認、手順書の作成、結果の記録の依頼の受け方までを扱います。手順書の雛形は
  同スキルの `template.md` です。
- 検証手順書は個別のBLに閉じず、次のリリースごとの `docs/verification/VERIFICATION-<バージョン>.local.md`
  （Git管理外）へBLごとの節として集めます。BL-208の手順書を `VERIFICATION-0.14.0.local.md` へ移しました。
- `CLAUDE.md`「本リポジトリの品質ゲート定義」の人手検証の記述を上記の分担へ改め、Claude Code が
  adbでUIを操作して検証するのはユーザーが明示的に求めた場合だけとしました。Skill
  `realmachine-verification` の説明と最重要原則もこれに合わせました。

## 2026-09-23（「更新」ボタンとデバイス名の位置の変更に伴う利用者向けドキュメントの追随）

タイル・ホーム画面ウィジェットの左上を「更新」ボタンにし、デバイス名と経路のマークを状態表示の
上の帯へ移した変更（BL-204〜BL-206）に合わせて、利用者向けドキュメントを更新しました（BL-207）。

- `docs/USER_GUIDE.md`: タイル・ウィジェットの構成表、状態の取り直し方（「デバイス名部分をタップ」→
  「更新をタップ」）、経路のマークの位置、「更新」でBluetoothの接続を確かめる動作の説明を更新しました。
- `docs/RELEASE_NOTES.md`: 0.14.0（未リリース）として変更点とストア掲載用の要約を追記しました。
  バージョン番号は機能追加のため0.13.0の次のマイナー版を仮に置いており、`scripts/version.properties`は
  リリース時に更新します。
- `README.md` / `docs/INSTALL.md`: 状態取得の操作を「更新」のタップへ直しました。
...
- **BL-197（完了・クローズ）**: 0.13.0向けのPlay Console掲載情報（スマートフォン用スクリーンショット
  7枚・詳細な説明）の差し替えが完了しました。BACKLOG.mdから削除しています。
- **BL-149（クローズ）**: Complicationの`LONG_TEXT`枠の表示確認とAndroid 11以下での既定の配置サイズ
  確認が残っていましたが、いずれも該当機材（該当枠を持つ文字盤／Android 11以下の端末）が無く検証手段が
  無いため、未確認のまま運用する方針としてクローズしました。BACKLOG.mdから削除し、DESIGN.mdの該当箇所を
  更新しています。
- **BL-195（保留）**: BLEが数分単位で届かなくなる時間帯の原因切り分け（Hub 3の電源を抜いての検証）は、
  単発の観測のみで再現頻度が未確認のため、現時点では積極的な調査を行わず、同様の症状が再発した際の
  ユーザーからの報告を契機に対応を検討する方針としました。BACKLOG.md上は残し、`タスク内容`と`根拠`に
  この方針を追記しています。
- **BL-198**: 優先度を下げる検討をしましたが、既に本リポジトリの優先度スケールの最下位（P3）のため
  変更していません。
...
前回の対策（スクリーンショットの切り出し・縮小）が効いて画像は約60枚から12枚程度まで減った一方、
**別の3つが利用量を押し上げている**ことが分かりました。

1. フィルタ無しの `logcat -d | grep` が1回で2,000トークン超の `WindowManager` 行を返した。
   `dumpsys connectivity` は1行で1,000トークンを超える
2. スマートフォンのWi-Fiデバッグが6回切断し、そのたびに利用者への依頼と再接続の往復が発生した
3. 検証ループが120秒のツール制限を超えてバックグラウンドへ移り、結果を読むやり取りが増えた

- **`verification-cost-control` スキルを新規作成**しました（ユーザースコープ、対象プロジェクトを
  問わない汎用）。何にいくらかかるかの目安、画像を使わずテキストで取る方法、
  `logcat` / `dumpsys` の絞り方、接続断による往復の削減、バックグラウンド実行の使い分け、
  検証手順そのものの組み立て方を持ちます。
- **`adb-device-connection` スキル**へ、長い検証ではUSB接続を最優先で依頼すること
  （機内モードを伴う検証ではUSBが必須）、Wi-Fiを繋ぎ直すとワイヤレスデバッグがOFFへ戻ること、
  mDNSの待ちは12秒程度必要で、取りこぼすデバイスは `ADB_MDNS_OPENSCREEN=1` で現れることを追加しました。
- **[.claude/skills/realmachine-verification/SKILL.md](.claude/skills/realmachine-verification/SKILL.md)**
  の第8節を汎用スキルへ委譲し、本リポジトリ固有の確認コマンド対応表だけを残しました。
  あわせて、機内モードにしてもWi-Fiが残ること、短時間にBLEスキャンを繰り返すとAndroidの
  スキャン制限で `NOT_FOUND` が続くこと、実Sesameの操作で公式アプリが出す通知を
  自アプリの通知と取り違えないことを追加しました。
- **[CLAUDE.md](CLAUDE.md)** の「検証コストの抑制」へ、出力を絞ること・USB接続を先に依頼することを
  追加し、手順の正本を `verification-cost-control` としました。
...
方針を変更しました**（ユーザー判断）。BLEで届かなかったときに利用者が原因を切り分けられないためです。
あわせて、セサミ本体の状態（電池残量・角度）を見られる画面を追加する方針を決め、
BL-166〜BL-172 として起票しました。

- 経路は、Tile・Complication・ウィジェットでは既存行へアイコン（📶 / ☁）を前置する形で示します。
  表示余白が無く、過去に文言が収まらず省略された事例（BL-102 / BL-104 / BL-158）があるため、
  **行は増やしません**。スマートフォンの画面では文言で明示します。
- BLEを試して届かずインターネット経由へ倒れた場合は、スマートフォンでトーストを出します。
  通知（Notification）ではなくトーストを選んだのは、Android 13 以降で POST_NOTIFICATIONS 権限の
  要求が増え、データセーフティ申告（BL-154）も増えるためです。
- 経路の方針（「自動（Bluetooth優先）」「常にインターネット経由」）を設定画面から選べるようにします。
  「Bluetooth固定」は、圏外で操作できなくなるため用意しません。
- **電池残量はBLE専用の情報ではありません。** Sesame Web API の状態取得レスポンスも
  `batteryVoltage` と `position` を返しており、これまで捨てていただけでした。電池表示は経路に
  よらず成立します。
