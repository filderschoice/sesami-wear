...
- **BL-163（起票のみ）**: 品質ゲートの定義が `CLAUDE.md` と `CONTRIBUTING.md`・
  `.github/PULL_REQUEST_TEMPLATE.md` の間で乖離しています（記録ファイルの YAML 検証が参照側に無い）。
  `CLAUDE.md` 自身が両方の更新を義務付けているため規約上の不整合にあたりますが、修正対象が
  ルールファイルと `.github/` 配下のため、ユーザーの承認を得てから反映します。
