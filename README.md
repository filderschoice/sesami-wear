...
施錠状態を表示できます。0.11.0以降は、スマートフォンのホーム画面ウィジェットからも施錠・解錠できます。

> **本アプリは個人による非公式プロジェクトです。** CANDY HOUSE株式会社とは提携・協力関係にありません。
...
- 状態を最後に取得した時刻を「3分前」「9/17」「未取得」のように表示（0.12.0以降）。状態の自動取得は
  行わず、施錠/解錠の成功時と、「更新」をタップしたときだけ更新する（0.13.0以前はデバイス名のタップ）
...
状態を出すモードです。デバッグ版（`Sesami Wear (debug)`）のスマートフォンアプリの設定メニュー
「撮影モード」から開きます。リリース版には含まれません。

1. 撮影モードの画面で「見本の3台に置き換える」を押し、スイッチをオンにする
2. デバイスごとに施錠状態・電池残量・経路・直近の失敗・最終取得時刻を選ぶ（ウォッチとウィジェットへすぐ反映される）
3. ウォッチのTileは撮影用デバイス（または「全デバイス」）へ設定し直す
4. 撮り終えたらスイッチをオフにする（実物の一覧と状態がウォッチへ同期し直される）

オンの間は施錠/解錠してもSesame Web APIもBluetoothも使わず、実物のセサミは動きません。
登録済みの資格情報と実物の状態は読み書きしません。撮影用デバイスはオンの間にカード一覧から
追加・編集・削除でき、APIキー・秘密鍵はダミー値に置き換えて保存されます。
設計は [DESIGN.md](docs/records/managed/DESIGN.md)「撮影モード（デバッグ版限定）」を参照してください。
...
- Sesame APIのレスポンス構造（`SesameStatus`）と施錠/解錠コマンドの署名・ペイロード仕様は、
  ...
  施錠/解錠の疎通確認は完了しています。
- Sesame Web API には1か月あたりのリクエスト回数の上限があります。上限に達すると、状態取得も
  施錠/解錠も HTTP 403 で失敗し、アプリ全体が機能停止します（2026-09-18 に実際に発生。応答は
  APIキーが無効な場合と同一で、アプリからは区別できません）。**上限の具体的な回数は利用者の環境での
  実測値（1000回）であり、公式ドキュメント上の記載は未確認です。** 当月の利用状況と上限は
  SESAME Biz（<https://biz.candyhouse.co>）で確認できます。Web APIのリファレンス
  （`doc.candyhouse.co/ja/SesameAPI/`）は 2026-09-18 時点で GitHub Pages の認証へリダイレクトされ、
  認証なしでは参照できません。
- 状態の自動取得は行いません。Sesame Web API には月間リクエスト上限があり、以前の「表示のたびに
  古ければ取得する」動作では上限を大きく超過して API キーが拒否され、アプリ全体が「状態不明」に
  なる事象が発生したためです（0.12.0 で廃止）。状態が更新されるのは、施錠/解錠が成功したときと、
  Tile・ウィジェットの「更新」をタップして明示的に更新したときだけです（0.13.0以前はデバイス名のタップ）。表示にはあわせて
  「3分前」「9/17」「未取得」のように最後に取得した時刻を表示します。
- そのため、Sesame純正アプリでの操作など他経路による状態変化は、利用者が明示的に状態を取得するまで
  反映されません。定期ポーリングも行いません。
