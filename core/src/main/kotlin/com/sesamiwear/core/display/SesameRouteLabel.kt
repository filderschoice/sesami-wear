...
object SesameRouteLabel {
    ...
    /**
     * Bluetoothで届かないときに、利用者が自分で試せる対処（BL-192）。
     *
     * Sesame 5はBLEの接続を同時に1つしか受け付けないため、**セサミ公式アプリが接続を掴んでいる間は
     * 本アプリの探索から見つけられないことがある**。2026-09-21の実機検証では、公式アプリを前面で
     * 開いてから約10分のあいだ探索がすべて`NOT_FOUND`になり、`am force-stop`で止めた直後に
     * 見つかるようになった（`NOT_FOUND`→`CONNECTION_FAILED`→以降9回連続で成功）。
     * ただし常に妨げるわけではなく、前面で開いたままでも成功する場合がある（同日、3回中2回）。
     * 利用者から見ると「急にBluetoothで届かなくなった」としか分からないため、[FALLBACK_MESSAGE]の
     * 通知と、ヘルプの「Bluetoothで届かないとき」で同じ対処を案内する。
     */
    const val OFFICIAL_APP_HINT =
        "セサミ公式アプリを開いていると、そのアプリがBluetoothの接続を掴み、" +
            "こちらから見つけられないことがあります。公式アプリを終了してからお試しください。"
