...
**実機で撮るときは撮影モード（デバッグ版のみ、BL-212 / BL-213）を使う。** スマートフォンのデバッグ版の
設定メニュー「撮影モード」でオンにすると、ウォッチ・ウィジェット・カード一覧が撮影用のデバイス
（見本は玄関・勝手口・ガレージ）と、画面で選んだ状態（施錠/解錠・電池・経路・失敗・取得時刻）に
差し替わる。実物のセサミは操作されず、登録済みの資格情報も触らないため、実デバイス名のモザイクも
実鍵の開け閉めも要らない。Tileは撮影用デバイスか「全デバイス」へ設定し直す。撮り終えたらオフへ戻す。
使い方の正本は `README.md`「撮影モード（デバッグ版のみ）」。
スマートフォンの実機で撮った画像（ホーム画面ウィジェット等）は、ステータスバー・ドック・壁紙に実機の
状態が写るため、Skill `store-screenshot-masking` でモザイク・ぼかしを掛けてから保存する。
