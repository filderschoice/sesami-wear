...
> **未転記の変更があります。** 0.11.0は2026-09-18にクローズドテストの両トラックで公開済みですが、
> ホーム画面ウィジェットを反映した短い説明・詳細な説明はPlay Consoleへ未転記です（BL-127）。
> リリースと同じ送信へ混ぜると差し戻し時にアプリ更新の配信まで止まるため、リリースの公開後に
> 別送信で反映します。転記が済んだらこの注記を削除してください。
...
| 短い説明 | 57 | 80 |
| 詳細な説明 | 1465 | 4000 |
| このリリースの新機能（`../RELEASE_NOTES.md` の最新バージョン） | 156 | 500 |
