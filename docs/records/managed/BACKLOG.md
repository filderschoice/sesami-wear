...
- id: BL-164
  区分: 人手検証
  タスク内容: >-
    0.12.0 のリリースタグ v0.12.0 と GitHub Releases を作成する。v0.9.0（BL-098）・
    v0.10.0（BL-116）・v0.11.0（BL-138）と同じ扱いで、クローズドテストの両トラックが
    「公開中」になってから作成する（2026-09-19 に公開中とストア掲載情報の反映を確認済み）。
    本文は docs/RELEASE_NOTES.md の 0.12.0 の内容を転記し、相対リンクを絶対 URL へ変換し、
    ストア掲載用の要約は除外する。一般公開前かつ `0.x` のため pre-release として公開する。
    タグが指すのは 0.12.0 の成果物を作ったコミット（BL-155 のリリースブランチのマージコミット）。
  ...
  担当: ユーザー
  完了条件: >-
    v0.12.0 の注釈付きタグが main に作成・push され、GitHub Releases に 0.12.0 の
    リリースノートを本文とするリリースが公開されていること
  根拠: >-
    タグの push と GitHub への発信を伴うため自律ループ実行モードでは実行できない
    （rules/guardrails-unified.v1.md セクション12.2）。BL-138 と同じ理由で、配信そのもの
    （BL-155）とは別項目として起票した。
  依存: []
...
- id: BL-150
  ...
  タスク内容: >-
    ...
    BLEのみで完結するためSesame Web APIのリクエストを消費しない（月間上限へ到達している状態でも
    実施できる。上限そのものは2026-09-19にプランの引き上げで解消済み）。
