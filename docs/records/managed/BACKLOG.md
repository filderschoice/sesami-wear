...
- id: BL-211
  区分: 人手検証
  タスク内容: >-
    BL-209の表示を実機で確認する。Tile（Pixel Watch）で長いデバイス名（例 10文字以上）を選んだとき、
    右上の帯で経路マークが名前の前に表示され、名前だけが末尾省略されること。左列の「更新」「変更」が
    1行に収まり、円形画面の縁で見切れないこと。ウィジェット（4x2）でも経路マークが名前の前にあること。
  優先度: P2
  状態: 未着手
  担当: ユーザー
  完了条件: >-
    上記をユーザーが実機で確認し、結果を記録していること。
  依存: []
  根拠: >-
    Tile・ウィジェットの描画はAndroid依存で単体テストの対象外のため、見た目は実機で確認する。
- id: BL-210
  区分: UX改善
  タスク内容: >-
  ...
  担当: AIエージェント
  完了条件: >-
    帯の背景をボタン（`CHIP_NEUTRAL_COLOR_ARGB`）より暗い色へ変え、帯のタップを外したものを
    ウィジェットとTileの両方へ反映し、Gradle系の品質ゲートがすべて成功していること。
    実機でボタンと区別できることの確認は人手検証として別途行う。
  依存: []
  根拠: >-
    2026-09-23にユーザーが「より暗い背景色」と「タップを外す」を選んだ。BL-209（完了）と同じ帯を触るため後に行った。
- id: BL-203
  区分: 人手検証
  タスク内容: >-
