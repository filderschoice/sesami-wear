...
- id: BL-214
  区分: 人手検証
  タスク内容: >-
    撮影モード（BL-212 / BL-213）を実機（Pixel 8 Pro + Pixel Watch 2、デバッグ版）で確認する。
    設定メニューの「撮影モード」から見本の3台を入れてオンにしたとき、(1) ウォッチのデバイス選択・Tile・
    Complicationとホーム画面ウィジェット・カード一覧が撮影用デバイスに差し替わること、(2) 操作画面で
    施錠状態・電池・経路・失敗・取得時刻を変えるとウォッチとウィジェットが追随すること、(3) Tile・
    ウィジェットから施錠/解錠しても実物のセサミが動かず、撮影用の状態だけが切り替わること、
    (4) カード一覧のタイトル横に「📷 撮影モード」が出ること、(5) オフへ戻すと実物の一覧・状態へ戻り、
    登録済みの資格情報が変わっていないことを見る
  優先度: P2
  状態: 未着手
  担当: ユーザー
  完了条件: >-
    (1)〜(5)が確認できていること。不具合があれば別途起票されていること
  根拠: >-
    Wear OS実機とのData Layer連携を要し、単体テストでは確かめられないため人手検証とする。
    BL-198の残り3点の撮影はこの確認を兼ねて行える
  依存: []
...
- id: BL-198
  ...
  タスク内容: >-
    ...
    **残りは`wear_1_complication.png`（文字盤の編集が必要）、`wear_4_tile_mixed.png`
    ...
    残り3点を作れる。
    2026-09-23にデバッグ版へ撮影モード（BL-212 / BL-213）を追加した。スマートフォン側から
    撮影用のデバイス一覧と状態を同期するため、上記の表示名の問題は起きず、1台だけ解錠した状態も
    実鍵を動かさずに作れる。残り3点は撮影モードで撮る（手順は`README.md`「撮影モード（デバッグ版のみ）」）。
  優先度: P3
  状態: 進行中
  担当: ユーザー
  完了条件: >-
