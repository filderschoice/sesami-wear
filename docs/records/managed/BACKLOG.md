...
- id: BL-151
  ...
  タスク内容: >-
    ...
    存在しないため、検証は実機必須）。
    着手条件だった2点の計測は2026-09-19に完了し、**公式SDKの取り込みは断念して自前実装を採る**
    ことが確定した（DESIGN.md「公式SDK取り込みの実測」）。依存アーティファクトが
    95個45.8MBから200個75.1MBへ増えること、Amplify（AWS Cognito）・AWS IoT・API Gatewayの
    資格情報を含むライブラリが`api`依存として不可避に付いてくること、Kotlinメタデータ2.2.0と
    本プロジェクトのKotlin 2.0.21が非互換でビルドが通らないことが理由。
    プロトコルは`meronepy/gomalock`・`homy-newfs8/libsesame3bt-core`（いずれもMIT）を参照する。
    AES-CMACは`core.crypto.AesCmac`を流用し、パケット組み立て・セッション確立・コマンド生成の
    ロジックはAndroid非依存のクラスへ切り出してユニットテストで検証する。
  ...
  根拠: >-
    ...
    2026-09-19に実証済みのため、依存は解消している。実装方式の決定に必要な計測は
    2026-09-19に完了している。実機を要する疎通確認は完了条件に含まれるため、
    実装完了後に人手検証項目を別途起票して本項目から切り離す。
  依存: []
