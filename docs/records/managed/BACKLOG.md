...
- id: BL-126
  ...
  タスク内容: >-
    ...
    2026-09-16 に (1)(2)(4)(5)(6)(9)(11) と (3) のうち対象選択部分を、Claude Code が adb 経由の
    UI 操作（スクリーンショットと logcat による確認）で実施済み。実資格情報を使わないため、
    debug 版を BL-131 の併存インストールで入れ、デモモードとダミー資格情報で検証した。
  ...
  完了条件: >-
    ...
    2026-09-16 時点で (3) の MIXED 表示・(8) のウィジェットと Tile の相互追随・(10) の
    Wear OS コンパニオン未導入スマホでの動作が未確認のため実機確認待ち（未確認）。
    (3)(8) は実 Sesame デバイスの状態が必要で、デモモードでは検証できない
    （デモの状態はウィジェットと Tile で意図的に独立しているため）
  根拠: >-
