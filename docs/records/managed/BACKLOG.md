...
- id: BL-152
  ...
  根拠: >-
    ...
    完了時に人手検証項目を起票する。BL-151の実装は2026-09-19に完了しており、依存は解消している。
    タイムアウトの設計値はBL-165の実測値で見直す。
  依存: []

- id: BL-165
  区分: 人手検証
  タスク内容: >-
    BL-151で実装したBLE直接操作を、デバッグビルドと実Sesameデバイスで検証する。
    ...
    あわせて、スキャンから応答までの所要時間を計測する。BL-152の経路選択は探索・接続に
    合計2秒程度しか割けない見込みのため（DESIGN.md「経路の優先順位と切り替え条件」）、
    実測値が設計値の妥当性を決める。
  優先度: P3
  状態: 未着手
  担当: ユーザー
  完了条件: >-
    上記(1)〜(4)を実機（Pixel 8 Pro + 登録済みのSesame 5）で確認し、スキャンから応答までの
    所要時間の実測値がDESIGN.mdへ記載されていること
  根拠: >-
    実資格情報（secretKey）と実Sesameデバイスを要するため、自律ループ実行モードの完了判定から
    除外する（rules/guardrails-unified.v1.md 12.5）。BL-151の完了条件のうち実機を要する部分を
    切り出したもので、実装そのものは2026-09-19に完了している。
  依存: []
