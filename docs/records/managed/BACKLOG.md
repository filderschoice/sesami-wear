...
- id: BL-117
  ...
  タスク内容: docs/store/STORE_LISTING.md の「詳細な説明」へ追記した3行（複数台の切り替え、
    「全デバイス」での一括操作、Sesame未登録時のデモ）と、「非公式アプリについて」へ追記した
    アプリ名「Sesami」の由来（公式製品Sesameをもじった造語であること）を、Google Play Consoleの
    ストア掲載情報へ転記する。実測文字数は同ファイル「文字数の実測値」のとおり1356文字（上限4000文字）。
