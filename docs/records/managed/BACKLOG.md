...
- id: BL-210
  区分: UX改善
  タスク内容: >-
    ウィジェット（4x2）とTileの右上のデバイス名の帯が、「更新」「変更」と同じ中立色のため押せるボタンに
    見える（BL-208の実機検証A-1でのユーザー指摘）。帯を表示用の見た目（例 背景を外す・枠線だけにする・
    より暗い色にする）へ寄せ、ボタンと区別できるようにする。帯のタップ（状態取得）を残すか外すかも
    あわせて決める。経路マークが緑・赤の上で見分けにくかった問題（BL-205 / BL-206）を再発させないこと。
  優先度: P3
  状態: 要確認
  担当: ユーザー
  完了条件: >-
    帯の見た目（色・背景の有無）とタップの扱いをユーザーが決め、ウィジェットとTileの両方へ反映し、
    実機でボタンと区別できることを確認していること。
  依存: []
  根拠: >-
    見た目の好みに関わる変更で、複数の案があるため、実装前にユーザーの選択を待つ。
- id: BL-209
  区分: 不具合
  タスク内容: >-
    Tileの右上のデバイス名の帯で、デバイス名が長いと経路マークが右端で見切れる（BL-208の実機検証A-3）。
    名前だけを省略して経路マークは常に表示されるようにする（例 名前の`Text`へ残り幅を与えて末尾省略する、
    アイコンを先頭へ置く）。あわせて左列（「更新」「変更」、`LEFT_COLUMN_WIDTH_DP`=76dp）の幅を狭め、
    右列（帯と状態チップ）へ幅を回す（同A-3でのユーザー指摘）。ウィジェット（4x2）は見切れの指摘が無かったため、
    変更する場合は同じ考え方で揃えるかを判断する。
  優先度: P2
  状態: 未着手
  担当: AIエージェント
  完了条件: >-
    長いデバイス名でもTileの経路マークが表示され、「更新」「変更」の2文字が左列に1行で収まり、
    Gradle系の品質ゲートがすべて成功していること。実機での見た目の確認は人手検証として別途行う。
  依存: []
- id: BL-203
  区分: 人手検証
  タスク内容: >-
