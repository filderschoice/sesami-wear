...
- id: BL-162
  区分: 品質ゲート
  タスク内容: >-
    `README.md`「主な機能」（17〜27行目）に、0.12.0で追加・変更した機能が反映されていない。
    `docs/RELEASE_NOTES.md`の0.12.0と`docs/USER_GUIDE.md`には記載があり、`README.md`だけが古い。
    未反映は次の5点。(1)最後に状態を取得した時刻の表示（`docs/USER_GUIDE.md` 176行目）、
    (2)取得・操作に失敗したときの理由表示（同193行目）、(3)ホーム画面ウィジェットの大きさ変更
    （同140行目）、(4)ウィジェット操作の振動（同150行目）、(5)資格情報設定画面の
    「今月のAPI呼び出し回数」の表示。既存のウィジェット項目に倣い「0.12.0以降」の注記を添える。
    アプリ内ヘルプの「APIのリクエスト回数の上限」は、`README.md`の「既知の未確認事項・制約」
    （233〜238行目）に上限そのものの説明が既にあるため、機能一覧へ重複して書かない。
  優先度: P2
  状態: 未着手
  担当: AIエージェント
  完了条件: >-
    `README.md`「主な機能」から0.12.0の追加機能が読み取れ、`docs/RELEASE_NOTES.md`・
    `docs/USER_GUIDE.md`・`docs/records/managed/DESIGN.md`と矛盾しないこと。
    `npx markdownlint-cli2 "**/*.md"`が0 issuesであること
  根拠: >-
    2026-09-18のドキュメント整合性レビュー（Skill `docs-consistency-review`の第3節「利用者向け
    ドキュメントに未反映の機能」）で検出した。README.mdは公開済みバージョンではなく現行コードの
    状態を記述しており（自動状態取得の廃止を制約側へ記載済み）、0.12.0の機能を書くことと矛盾しない。
  依存: []
...
- id: BL-150
  ...
  タスク内容: >-
    ...
    PCから確認できる。**実施手順は`docs/BLE_KEY_VERIFICATION.md`にある**（準備・スキャン・
    状態取得のスクリプト・クラウド未接続の確認方法・切り分け・戻す内容。BL-161で作成）。
    BLEのみで完結するためSesame Web APIのリクエストを消費せず、月間上限（BL-141）に
    到達している状態でも実施できる。
