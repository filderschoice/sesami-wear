...
- **操作画面（BL-213）。** デバッグ版の設定メニューだけに「撮影モード」（副題にオン/オフ）が出て、
  `ShowcaseModeActivity`（`src/debug/AndroidManifest.xml`で宣言、`exported="false"`）を開く。
  モードのスイッチ、「見本の3台に置き換える」「全台を施錠中」「全台を解錠中」と、デバイスごとに
  施錠（施錠中/解錠中/未取得）・電池（100/85/50/20/5%/不明）・経路（Bluetooth/インターネット/不明）・
  失敗（なし/認証/通信/端末設定）・取得（たった今/5分前/1時間前/1日前）のチップを並べる。
  選択肢は`ShowcaseStateOptions`（取得時刻は保存後も進むため、経過時間以下で最も大きい選択肢を選択中とする）。
  未取得のときは電池・経路・取得時刻を保存できない（施錠状態と一緒にしか残らない）ため押せなくする。
  1項目を変えると、保存値から戻した状態（`ShowcaseDeviceState.of`）のその項目だけを変えて丸ごと書き直す。
- **モードの表示（BL-213）。** 撮影モード中だけ、カード一覧のタイトルの横へ小さく「📷 撮影モード」を出す
  （`ShowcaseModeUi.Indicator`）。撮影の邪魔にならないよう帯にはせず、ウォッチとウィジェットには出さない
  （ユーザーの選択）。`MainActivity`は作ったときのモードを覚えておき、切り替えて戻ってきたら作り直す
  （画面は作ったときの保存先を持ち続けるため）。
- メニュー項目と表示も`ShowcaseModeUi`（`src/debug`に実装、`src/release`に何も出さないスタブ）越しに
  呼ぶ。メニュー項目の型`ShowcaseMenuEntry`だけはmainに置く。
- 撮影用の実行口・保存先・選択肢のテストは`src/testDebug`（デバッグ版の単体テストにだけ含まれる）。
- 実機検証（BL-214、2026-09-23、Pixel 8 Pro + Pixel Watch 2 + 登録済みのSesame 5、**ユーザーが実施**）:
  インストールと手順書をClaude Code、操作と判定をユーザーが担当し、15項目すべてで期待どおりに動いた。
  - 切り替えと表示: 設定メニューの「撮影モード」はデバッグ版だけに出た。見本の3台を入れてオンにすると、
    カード一覧が撮影用の3台に替わり、タイトル横に「📷 撮影モード」が出た。ウォッチのデバイス選択には
    表示名で並び（BL-198の`SesameWearDebugReceiver`で起きた、名前ではなくuuidが出る問題は起きない）、
    Tileは「スマホ未接続」にならずに撮影用の状態を表示した。ウィジェットも撮影用デバイスを表示した。
  - 状態の切り替え: 施錠状態・電池・経路・取得時刻・失敗の変更にタイルとウィジェットが数秒以内に追随した。
    失敗を設定した状態ではタイルからの施錠が失敗のハプティクスになり、状態は変わらなかった。未取得では
    電池・経路・取得のチップが押せなくなった。「全台を施錠中」「全台を解錠中」も期待どおりだった。
  - 操作の抑止: タイルとウィジェットから施錠/解錠すると撮影用の状態だけが切り替わり、**実物のセサミは
    動かなかった**。カード一覧から撮影用デバイスを追加・編集・削除でき、ウォッチにも反映された。
    保存後のAPIキーと秘密鍵はダミー値へ置き換わっていた。
  - 元に戻す: オフにするとカード一覧とウォッチのデバイス選択が実物の一覧に戻った。タイルとウィジェットは
    実物のデバイスへ設定し直すと、撮影モード前の状態を表示した。実物のデバイスの「更新」も通常どおり
    成功し、資格情報が変わっていないことを確認した。
